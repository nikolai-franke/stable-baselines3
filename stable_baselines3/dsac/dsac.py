...
from stable_baselines3.dsac.policies import DiscreteCnnPolicy, DSACPolicy, MlpPolicy
...
class DSAC(OffPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if self.target_entropy == "auto":
            ...
            #     np.float32
            # )
            self.target_entropy = 0.99 * -np.log(1 / self.env.action_space.n)
        else:
            # Force conversion
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_q1, next_q2 = self.critic_target(replay_data.next_observations)
            ...
            current_q_values = th.stack(self.critic(replay_data.observations))
            critic_loss = 0.5 * sum(
            ...
            # q_values_pi = th.cat(self.critic(replay_data.observations), dim=0)
            with th.no_grad():
                q_values = th.stack(self.critic(replay_data.observations))
